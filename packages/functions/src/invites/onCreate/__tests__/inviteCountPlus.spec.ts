...
import inviteCountPlus from "../inviteCountPlus";

const testEnv = functions();
const db = admin.firestore();

test("On space item create, increase the space inviteCount", async done => {
  spyOn(db.doc(""), "update").and.returnValue("updated");

  const params = {spaceId: "spaceId"};
  const wrapped = testEnv.wrap(inviteCountPlus);
